...
class KNN:
    ...
    def predict(self, features):
        ...
        for i in range(0,len(features)):
            k_neighbors = self.get_k_neighbors(features[i])
            s = sum(k_neighbors)
    ...
    def get_k_neighbors(self, point):
        ...
        neighbors = sorted(neighbors, key = lambda l:l[1])
        
        answers = []
        for i in [row[0] for row in neighbors[0:self.k]]:
            answers.append(self.labels[i])
