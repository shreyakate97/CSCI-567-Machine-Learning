...
class HMM:
    ...
    def forward(self, Osequence):
        ...
        for t in range(1,L):
            for s in range(0,S):
                summation = 0
                for s_dash in range(0,S):
                    summation += self.A[s_dash,s]*alpha[s_dash,t-1]
    ...
    def backward(self, Osequence):
        ...
        for t in range(L-2,-1,-1):
            for s in range(0,S):
                for s_dash in range(0,S):
                    beta[s,t] += self.A[s,s_dash]*self.B[s_dash,self.obs_dict[Osequence[t+1]]]*beta[s_dash, t+1]
        ###################################################
        return beta
    ...
    def posterior_prob(self, Osequence):
        ...
        s_prob = self.sequence_prob(Osequence)
       
        prob = (np.multiply(alpha,beta))/s_prob
        ###################################################
        return prob
    #TODO:
        ...
        for s in range(0,S):
            for s_dash in range(0,S):
                for t in range(0,L-1):
                    prob[s,s_dash,t] = alpha[s,t]*self.A[s,s_dash]*self.B[s_dash, self.obs_dict[Osequence[t+1]]]*beta[s_dash,t+1]
    ...
    def viterbi(self, Osequence):
        ...
        for t in range(1,L):
            for s in range(0,S):
                m = np.zeros(S)
                for s_dash in range(0,S):
                    m[s_dash] = self.A[s_dash,s]*delta[s_dash,t-1]
                maxi = np.amax(m)
                delta[s,t] = self.B[s, self.obs_dict[Osequence[t]]]*maxi
                
                tri[s,t] = int(np.argmax(m))
        ...
        path.append(key_list[val_list.index(int(np.argmax(delta[:,L-1])))])
                                         
        for t in range(L-1,0,-1):
            path.insert(0, key_list[val_list.index(tri[self.state_dict[path[0]],t])])
