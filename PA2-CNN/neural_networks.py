...
class linear_layer:
    ...
    def __init__(self, input_D, output_D):
        ...
        mean = 0
        std = 0.1
        self.params['W']  = np.random.normal(mean,std, (input_D, output_D))
        self.params['b'] = np.random.normal(mean,std, (1, output_D))
    ...
    def backward(self, X, grad):
        ...
        self.gradient['b'] = np.ones((1, len(X))).dot(grad)
...
class relu:
    ...
    def backward(self, X, grad):
        ...
        backward_output = np.multiply(grad, np.int64(self.mask > 0))
...
def main(main_params, optimization_type="minibatch_sgd"):
    ...
    for t in range(num_epoch):
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            # Do not modify them.
            ######################################################################################
            grad_h1 = model['L2'].backward(d1, grad_a2)
            grad_b1 = model['drop1'].backward(h1, grad_h1)
            grad_a1 = model['nonlinear1'].backward(a1, grad_b1)
